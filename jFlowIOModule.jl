...
function ReadParams()::Params
    ...
    mesh = Bool(data[7, 2])                         # externally generated mesh (if == true)
    params = Params(endTime, dt0, dtMax, dhMax, f, c, mesh)
    println("Read model run parameters.")
    return params
end


function ReadCells1(material::Array{Material, 1})::Array{Cell, 1}
    # read grid properties from file generated by external grid generator
    cell = Cell[]
    data = readdlm("nodes.csv", ',', header=true)
    for iRow = 1:size(data[1], 1)
        cellIndex = Int64(data[1][iRow, 1])
        x = Float64(data[1][iRow, 2])               # location of volume element/cell node point
        y = Float64(data[1][iRow, 3])
        z = Float64(data[1][iRow, 4])
        vol = Float64(data[1][iRow, 5])             # cell volume
        b = Float64(data[1][iRow, 6])               # cell vertical thickness
        ...
        connects = Int64[]
        neighbors = Int64[]
        push!(cell, Cell(cellIndex, x, y, z, vol, b, h-z, 0.0, mat, connects, neighbors))
        cell[end].S = Sat(cell[end])                # calculate initial saturation
    end
    println("Read and process externally-generated node assembly.")
    return cell
end


function ReadCells2(material::Array{Material, 1})::Array{Cell, 1}
    # read cell setup file and process cell objects from raw constraints
    cell = Cell[]
    data = readdlm("Cells.txt", '\t', header=true)
    for iRow = 1:size(data[1], 1)
        cellIndex = Int64(data[1][iRow, 1])
        x0 = Float64(data[1][iRow, 2])              # location of volume element/cell node point
...
function ReadGridConnects1()::Array{Connection, 1}
    # read grid cell connection parameters from file generated by external grid generator
    connect = Connection[]
    data = readdlm("connections.csv", ',', header=true)
    for iRow = 1:size(data[1], 1)
        cell1 = Int64(data[1][iRow, 1])                         # connected cells index numbers
        cell2 = Int64(data[1][iRow, 2])             
        del1 = Float64(data[1][iRow, 3])                        # distances from cell centerpoints to interface
        del2 = Float64(data[1][iRow, 4])
        A = Float64(data[1][iRow, 5])                           # connection interface area       
        zConnect = Bool(data[1][iRow, 6])                       # is this a vertical connection
...
function ReadGridConnects2()::Array{Connection, 1}
